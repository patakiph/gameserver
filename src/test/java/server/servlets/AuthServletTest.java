...
public class AuthServletTest {
    ...
    public class parseLeaderboard {
        private int user_id;
        private int score;
        private String login;

        public parseLeaderboard(int user_id, String login, int score) {
    ...
    public void test09_changeNameTest() throws Exception {
        ...
        RequestBody body = RequestBody.create(
                mediaType,
                String.format("name=%s", NEW_NEW_USER_NAME)
        );
        System.out.println(this.getTestToken());
...
//
    ...
    public void testLeaderboard() throws Exception {
        ...
        Type listType = new TypeToken<List<parseLeaderboard>>(){}.getType();
        Map jsonJavaRootObject = new Gson().fromJson(response.body().string(), Map.class);
        List<parseLeaderboard> users = new Gson().fromJson(jsonJavaRootObject.get("leaderboard").toString(), listType);
        List<String> nameList = users.stream().map(parseLeaderboard::getLogin).sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList());
        System.out.println(nameList);
